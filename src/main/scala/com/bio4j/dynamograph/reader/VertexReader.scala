...
class VertexReader[VT <: AnyDynamoVertex,R <: AnyRegion](val vertexType : VT ,val vertexTable : VertexTable[VT,R], val dbExecutor : AnyDynamoDbExecutor) extends AnyVertexReader{
  type VertexType = VT
